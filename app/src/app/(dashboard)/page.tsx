...
export default function TestExternalAPI() {
  ...
  const {
    ...
    isLoading,
    error,
  } = api.post.getUserFromExternalAPI.useQuery({
    userId: '1',
  });

  if (isLoading) return <div className="p-4">Consultando API externa...</div>;

  if (error)
    return (
      <div className="p-4 text-red-500">Error de tRPC: {error.message}</div>
    );

  return (
    <div className="p-4 border rounded shadow-sm max-w-sm">
      <h2 className="text-xl font-bold mb-2">Datos desde JSONPlaceholder:</h2>
      {/* Al escribir "user.", TypeScript te autocompletará id, name, username y email */}
      ...
      <p className="mt-4 text-xs text-gray-500">
        (Abre la consola de red en tu navegador. Verás que tu frontend hizo una
        petición a tu propio servidor Next.js, no directamente a
